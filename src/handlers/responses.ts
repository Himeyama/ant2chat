  ...
  if (body.stream) {
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const enqueue = (event: ResponsesStreamEvent) =>
        ...
        // ツール呼び出し: toolCallId → { id, callId, name, outputIndex, argsText, finalArgs }
        ...
        // output_index → 完成アイテム (response.completed 用)
        ...
        try {
            ...
            switch (part.type) {
              case "text-delta": {
                if (textItemId === null) {
                  ...
                  enqueue({ type: "response.output_item.added", output_index: textOutputIndex, item: msgItem });
                  enqueue({
              ...
              case "tool-call": {
                ...
                if (!entry) {
                  // ストリーミング非対応プロバイダー: tool-call のみ来るケース
                  const fcId = makeFcItemId();
                  const outputIndex = nextOutputIndex++;
          ...
          if (textItemId !== null && textOutputIndex !== null) {
            ...
            enqueue({ type: "response.output_text.done", item_id: textItemId, output_index: textOutputIndex, content_index: 0, text: textAccumulated });
            enqueue({ type: "response.content_part.done", item_id: textItemId, output_index: textOutputIndex, content_index: 0, part: doneContent });
            const doneMsg: ResponseOutputMessage = { type: "message", id: textItemId, role: "assistant", status: "completed", content: [doneContent] };
            enqueue({ type: "response.output_item.done", output_index: textOutputIndex, item: doneMsg });
            outputItems.set(textOutputIndex, doneMsg);
          }

          // ツール呼び出しアイテムのクローズ (output_index 順)
          ...
          for (const entry of sortedTools) {
            ...
            enqueue({ type: "response.function_call_arguments.done", item_id: entry.id, output_index: entry.outputIndex, arguments: argsStr });
            ...
            enqueue({ type: "response.output_item.done", output_index: entry.outputIndex, item: doneFc });
